...
export default function PeakPathApp() {
  ...
  // Check for sessions on component mount
  React.useEffect(() => {
    ...
    // First, check for existing localStorage session
    const existingSession = SessionManager.loadSession();
    if (existingSession) {
      console.log('Loading existing session:', existingSession.sessionId);
      setStrengthProfile(existingSession.profile);
      setIsDemoMode(existingSession.isDemoMode);
      setCurrentSessionId(existingSession.sessionId);
      setAppState('dashboard');
      
      // Load chat history
      const chatHistory = SessionManager.loadChatHistory(existingSession.sessionId);
      setChatMessages(chatHistory);
      return;
    }

    // Check for new upload from sessionStorage (from landing page)
    const savedProfile = sessionStorage.getItem('strengthProfile');
    ...
    if (savedProfile) {
      try {
        ...
        const sessionId = SessionManager.saveSession(profile, false);
        setCurrentSessionId(sessionId);
        
        // Clear sessionStorage
        sessionStorage.removeItem('strengthProfile');
      } catch (error) {
  ...
  React.useEffect(() => {
    if (currentSessionId && chatMessages.length > 0) {
      SessionManager.saveChatHistory(currentSessionId, chatMessages);
    }
  ...
  const handleSessionLoad = useCallback((sessionId: string) => {
    ...
    if (session) {
      setStrengthProfile(session.profile);
      setIsDemoMode(session.isDemoMode);
      setCurrentSessionId(sessionId);
      setAppState('dashboard');
      
      // Load chat history
      const chatHistory = SessionManager.loadChatHistory(sessionId);
      setChatMessages(chatHistory);
  ...
  const handleSessionDelete = useCallback((sessionId: string) => {
    if (sessionId === currentSessionId) {
      // If deleting current session, reset to upload state
      handleReset();
    }
  }, [currentSessionId, handleReset]);
  ...
  const renderContent = () => {
    switch (appState) {
      ...
      case 'dashboard':
        return (
          <div className={`flex flex-col h-screen ${isChatExpanded ? 'overflow-hidden' : ''}`}>
            {/* Session Banner */}
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 text-sm">
                  {isDemoMode ? (
                    <>
                      ...
                      <span className="font-medium text-purple-700">Demo Mode Active</span>
                      <span className="text-xs text-purple-600">- This is sample data for demonstration</span>
                    </>
                  ) : (
                    <>
                      <div className="w-2 h-2 bg-green-500 rounded-full"></div>
                      <span className="font-medium text-green-700">Session Active</span>
                      <span className="text-xs text-green-600">- Your data is saved locally</span>
                    </>
                  )}
                </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  {isDemoMode && (
                    <button 
                      onClick={handleReset}
                      className="text-xs bg-purple-200 hover:bg-purple-300 px-2 py-1 rounded-md transition-colors text-purple-700"
                    >
                      Exit Demo
                    </button>
                  )}
            ...
            <div className={`flex flex-1 ${isChatExpanded ? 'overflow-hidden' : ''}`}>
            {/* Expanded Chat Panel */}
            {isChatExpanded && (
